...
public class SectionOne {
    private static final Logger _LOG = Logger.getLogger(SectionOne.class.getName());
    ...
    public static void fileWriterAndReaderExample() {
        char[] in = new char[50];
        int size = 0;
        try {
    ...
    public static void consoleObj() {
        _LOG.info("[ENTER consoleObj(): void]");
        Console console = System.console();
        String name = "";
        if (console != null) {
            name = console.readLine("%s", "Whats your favorite Song?: ");
            console.format("%s\n", new MyUtility().doStuff(name));
            
            char[] pass = console.readPassword("%s", "Enter your pass: ");
            for(char c : pass) 
                console.format("%s", c + " ");
            console.format("\n");
        }
